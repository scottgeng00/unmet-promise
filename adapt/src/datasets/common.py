...
class ImageTextDatasetWrapper(torch.utils.data.Dataset):
    def __init__(self, dataset, captions, tokenizer=None, seed=42):
        super().__init__()
        self.dataset = dataset
        self.captions = captions
        self.rng = np.random.default_rng(seed)
        self.tokenizer = tokenizer
        assert len(self.dataset) == len(self.captions)
...
def get_features(is_train, image_encoder, dataset, device):
    ...
    if hasattr(dataset, 'split_maps'):
        subsplit_names_and_dataloaders = dataset.split_maps[split]
    else:
        subsplit_names_and_dataloaders = [(f"{dname}/{split}", dataset.train_loader if is_train else dataset.test_loader)]

    data = dict()
    for subsplit_name, subsplit_loader in subsplit_names_and_dataloaders:
        if image_encoder.cache_dir is not None:
            cache_dir = f'{image_encoder.cache_dir}/{subsplit_name}'
        ...
        if image_encoder.cache_dir is not None and len(cached_files) > 0:
            ...
            for cached_file in cached_files:
                ...
                cached_data = torch.load(cached_file)
                if name not in data:
                    data[name] = cached_data
                else:
                    data[name] = np.concatenate([data[name], cached_data], axis=0)
        else:
            print(f'Did not find cached features at {cache_dir}. Building from scratch.')
            subsplit_data = get_features_helper(image_encoder, subsplit_loader, device)
            for key, val in subsplit_data.items():
                if key not in data:
                    data[key] = val
                else:
...
def get_dataloader(dataset, is_train, args, image_encoder=None, test_loader=False, pin_memory=False):
    if image_encoder is not None:
        feature_dataset = FeatureDataset(is_train, image_encoder, dataset, args.device)
        dataloader = DataLoader(feature_dataset, batch_size=args.batch_size, shuffle=is_train, pin_memory=pin_memory)
    else:
